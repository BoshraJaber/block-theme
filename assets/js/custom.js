...
jQuery(document).ready(function () {
    ...
    /*get_album_data('all',1);
    jQuery('#album_date').on('change', function($) {
        var publish_date = this.value;
        get_album_data(publish_date,1);
    });
    jQuery('.album_section').on("click", '.ajax-kipic-pagination li', function(){
        var publish_date = jQuery('#album_date').val();
        var page = jQuery(this).attr('p');
        get_album_data(publish_date,page);
    });*/

    /*get_video_data('all',1);
    jQuery('#video_date').on('change', function($) {
        var publish_date = this.value;
        get_video_data(publish_date,1);
    });
    jQuery('.video-section').on("click", '.ajax-kipic-pagination li', function(){
        var publish_date = jQuery('#video_date').val();
        var page = jQuery(this).attr('p');
        get_video_data(publish_date,page);
    });*/
    ...
    jQuery(".tab-heading-block li .tab-link").click(function (e) {
        ...
        if (jQuery(window).width() < 768) {
            ...
            // jQuery(this).closest('.tabbing-outer-wrapper').find('.accordian-outer').removeClass('active');
            //jQuery(this).closest('.tabbing-outer-wrapper').find('.accordian-outer .accordian-content').slideUp();
            setTimeout(function () {
...
jQuery(document).ready(function () {
    //setTimeout(jQuery("html[lang='en-US'] a.jae_report_box.ar_view").remove(),5000);
    //setTimeout(jQuery("html[dir='rtl'] a.jae_report_box.en_view").remove(),5000);
    //jQuery("html[lang='en-US'] a.jae_report_box.ar_view").remove();
    //jQuery("html[dir='rtl'] a.jae_report_box.en_view").remove();
});
